...
def get_original_attribute(feature, attributes):
...
def main():
    ...
    feature_importance["attribute"] = feature_importance["feature"].apply(
        lambda feature: get_original_attribute(
            feature,
            simple_attributes_list
        )
    )

    attribute_importance = aggregate_feature_importance(feature_importance)

    # Get the top 10 restaurant attributes
    top_attributes = attribute_importance.head(10).index.tolist()

    # Calculate average rating and restaurant count for each value within the top 10 attributes
    results = []
